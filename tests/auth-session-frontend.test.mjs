...
function browserSession() {
  const values = new Map([
    ["impulse.auth.accessToken", "expired-access"],
    ["impulse.auth.refreshToken", "valid-refresh"],
    [
      "impulse.auth.user",
      JSON.stringify({
        id: "u1",
...
function loadApi() {
  for (const path of [
    require.resolve("../services/api.ts"),
    require.resolve("../services/session.ts"),
  ])
...
test("a single refresh renews tokens and retries every concurrent 401 request", async () => {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (count === 1)
      return {
        ok: false,
        status: String(url).endsWith("/users") ? 419 : 401,
