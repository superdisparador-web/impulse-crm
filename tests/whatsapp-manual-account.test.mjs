...
async function loadTypescriptModule(path, dependencies = {}) {
  ...
  Function(
    ...
    "exports",
    code,
  )(localRequire, moduleRecord, moduleRecord.exports);
  return moduleRecord.exports;
}

async function serviceWithApi(api) {
  return loadTypescriptModule("../services/whatsapp.service.ts", {
    "./api": { api },
  });
}
...
test("serviço envia cadastro manual somente no corpo do endpoint administrativo", async () => {
  ...
  const payload = {
    ...
    accessToken: "secret-token-value-long",
    apiVersion: "v23.0",
  };
  await whatsappService.createManualAccount(payload);
  assert.deepEqual(calls, [
    { path: "/whatsapp/admin/accounts/manual", body: payload },
  ]);
...
test("Embedded Signup permanece usando seu endpoint e não recebe credenciais manuais", async () => {
  ...
  const api = {
    post: async (path, body) => {
      call = { path, body };
      return { authorizationUrl: "https://meta.example", expiresAt: "soon" };
    },
  };
  try {
    const { whatsappService } = await serviceWithApi(api);
    ...
    assert.deepEqual(call, {
      path: "/whatsapp/embedded-signup/session",
      body: { returnUrl: "https://crm.example.test/whatsapp" },
    });
