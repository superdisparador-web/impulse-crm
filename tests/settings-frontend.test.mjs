...
const source = readFileSync(
  new URL("../components/settings/SettingsCenter.tsx", import.meta.url),
  "utf8",
);
const seed = readFileSync(
  new URL("../backend/prisma/seed.js", import.meta.url),
  "utf8",
);

const permissionMapCodes = [
  ...source
    .slice(
...
test("technical permission and notification names are translated before rendering", () => {
  ...
  for (const label of [
    "Ver informações da própria conta",
    "Receber notificações pelo aplicativo",
...
test("permission map translates every unique code returned by the permission matrix", () => {
  ...
  assert.deepEqual(
    [...permissionMapCodes].sort(),
    [...new Set(apiPermissionCodes)].sort(),
  );
});
...
test("Empresa and Aparência have independent form identities and fields", () => {
  ...
  const organization = source.slice(
    source.indexOf("function Organization"),
    source.indexOf("function Appearance"),
  );
...
test("security and errors do not promise unsupported features or expose technical failures", () => {
  assert.match(
    source,
    /title: 'Autenticação em dois fatores'.*status: 'Em breve'/,
  );
