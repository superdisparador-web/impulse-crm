...
const [page, table, form, sidebar] = await Promise.all([
  readFile(
    new URL("../components/users/UsersPage.tsx", import.meta.url),
    "utf8",
  ),
  readFile(
    new URL("../components/users/UserTable.tsx", import.meta.url),
    "utf8",
  ),
  readFile(
    new URL("../components/users/UserForm.tsx", import.meta.url),
    "utf8",
  ),
  readFile(
...
test("drawer e navegação expõem a Central de Usuários", () => {
  ...
  assert.ok(
    sidebar.indexOf('title: "Corretores"') <
      sidebar.indexOf('title: "Usuários"'),
  );
});
