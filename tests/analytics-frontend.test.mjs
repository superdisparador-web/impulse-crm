...
test('analytics dashboard provides loading, error, KPIs, rates, ranking and alerts', () => {
  ['Carregando inteligência comercial', 'Não foi possível carregar', 'Campanhas ativas', 'CTR', 'Read rate', 'Delivery rate', 'Ranking de campanhas', 'Alertas operacionais'].forEach((label) => assert.ok(page.includes(label)));
});
