...
test("campaign dashboard consumes the existing campaign funnel endpoint and renders every stage", () => {
  ...
  [
    ...
    "Tempo médio",
  ].forEach((label) => assert.ok(component.includes(label)));
...
test("broker and manager dashboards share the same responsive implementation", () => {
  ...
  ].forEach((label) => assert.ok(dashboard.includes(label)));
...
test("visual timeline consumes the existing lead timeline with filters and progressive pagination", () => {
  ...
  [
    ...
    "Mensagem lida",
    "Link acessado",
  ].forEach((label) => assert.ok(timeline.includes(label)));
});
...
test("reports expose visual commercial filters and safe CSV while leaving XLSX disabled", () => {
  ...
  [
    ...
    "Exportar CSV",
    "Exportar XLSX",
  ].forEach((label) => assert.ok(reports.includes(label)));
...
test("template analytics and events provide charts, search, ordering and pagination", () => {
  ...
  [
    ...
    "Última utilização",
  ].forEach((label) => assert.ok(templates.includes(label)));
  [
    "Pesquisar eventos",
    "Tipo do evento",
    "Mais recentes",
  ...
  ].forEach((label) => assert.ok(events.includes(label)));
});
...
test("professional chart primitives cover line, area, bars, pie, funnel and heatmap without a new dependency", () => {
  ...
  [
    ...
    "linearGradient",
    "conic-gradient",
  ].forEach((primitive) => assert.ok(charts.includes(primitive)));
});
