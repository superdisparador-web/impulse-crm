...
const embedded = readFileSync(
  "backend/src/whatsapp/meta/meta-embedded-signup.service.ts",
  "utf8",
);
const whatsappModule = readFileSync(
...
test("both connect buttons execute the same Embedded Signup service action", () => {
  ...
  assert.ok(
    (ui.match(/onClick=\{p\.onConnect\}|onClick=\{connect\}/g) ?? []).length >=
      2,
  );
});
...
test("frontend posts to the authenticated Embedded Signup endpoint", () => {
  assert.match(
    service,
    /api\.post<[^>]+>\('\/whatsapp\/embedded-signup\/session'/,
  );
...
test("backend exposes and registers the session endpoint without a global prefix", () => {
  ...
  assert.match(whatsappModule, /providers:[\s\S]*MetaEmbeddedSignupService/);
...
test("configuration and return origin are validated before creating a signed signup session", () => {
  assert.match(embedded, /async createSession/);
  assert.match(embedded, /returnUrl\.origin!==allowedOrigin/);
  assert.match(embedded, /signupConfiguration\(user,dto\.accountId\)/);
  for (const key of [
    "META_APP_ID",
    "META_APP_SECRET",
...
test("indicators are derived from returned account data instead of fixed values", () => {
  ...
  assert.match(
    ui,
    /const count = \(value: WhatsappAccountStatus\) => active\.filter/,
  );
