...
const page = readFileSync(new URL('../app/whatsapp/page.tsx', import.meta.url), 'utf8');
const modal = readFileSync(new URL('../components/whatsapp/AccessTokenModal.tsx', import.meta.url), 'utf8');
const view = readFileSync(new URL('../components/connections/ConnectionsEnterprise.tsx', import.meta.url), 'utf8');
const service = readFileSync(new URL('../services/whatsapp.service.ts', import.meta.url), 'utf8');
...
test('frontend traduz access token inválido conforme especificação', () => {
  assert.match(page, /WHATSAPP_INVALID_ACCESS_TOKEN.*O Access Token informado é inválido ou expirou\./s);
