...
export function getPipelineBoard(
  ...
  return api.get<PipelineBoard>(
    `/pipeline/${pipelineId}/board${query.size ? `?${query}` : ""}`,
  );
}
...
export function movePipelineCard(
  ...
  stageId: string,
  position: number,
): Promise<PipelineBoard | { ok: boolean }> {
  const payload: PipelineMovePayload = { stageId, position };
  return api<PipelineBoard | { ok: boolean }>(
...
export function updatePipelineStage(
  ...
  return api<PipelineStage>(`/pipeline/${pipelineId}/stages/${stageId}`, {
    method: "PATCH",
...
export function reorderPipelineStages(
  ...
  return api<PipelineStage[]>(`/pipeline/${pipelineId}/stages/reorder`, {
    method: "PATCH",
    body: JSON.stringify({
