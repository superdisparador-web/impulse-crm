import { api } from './api';
import { AddRecipientsPayload, Campaign, CampaignAudienceEstimate, CampaignAudienceMaterializationResult, CampaignCostEstimate, CampaignEstimateResponse, CampaignFilter, CampaignFilters, CampaignListResponse, CampaignProgressMetrics, CampaignSegmentationFilters, CampaignValidationResult, CreateCampaignPayload, OperationalRecipients, UpdateCampaignPayload } from '@/types/campaign';
...
class CampaignsService {
  ...
  validateCampaign(id:string){return api.post<CampaignValidationResult>(`/campaigns/${id}/validate`,{});}
  ...
  progress(id:string){return api.get<CampaignProgressMetrics>(`/campaigns/${id}/progress`);}
  ...
  configureDestination(id:string,data:CampaignDestinationConfiguration){return api<Campaign>(`/campaigns/${id}/destination`,{method:'PATCH',body:JSON.stringify(data)});}
...
export type CampaignDestinationConfiguration={mode:'FIXED_URL'|'AGENT_FIXED'|'ROUND_ROBIN';fixedUrl?:string;agentId?:string;agents?:CampaignAgentConfiguration[];initialIndex?:number;queue?:string;speed?:number;concurrency?:number;automaticDistribution?:boolean;timezone?:string};
