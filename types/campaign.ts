...
export type CampaignFilterOperator =
  "equals" | "in" | "between" | "gte" | "lte" | "contains" | "is";
...
export interface Campaign extends Partial<CampaignMetrics> {
  id: string;
...
export interface CampaignDurationEstimate {
  messages: number;
...
export interface CampaignCostEstimate extends CampaignDurationEstimate {
...
export interface CampaignAudienceEstimate {
  ...
  invalidosRemovidos: number;
  semTelefone: number;
  filtrosAplicados: CampaignSegmentationFilters;
  operation: CampaignCostEstimate;
}

export interface CampaignAudienceMaterializationResult extends Omit<
...
export interface CampaignValidationResult {
  valid: boolean;
  reasons: CampaignValidationIssue[];
  issuesByStep: Record<string, CampaignValidationIssue[]>;
  validRecipients: number;
  status: CampaignStatus;
}

export interface CampaignProgressMetrics extends CampaignProgress {
  clicked?: number;
...
export interface OperationalRecipients {
  items: OperationalRecipient[];
  meta: {
    page: number;
    limit: number;
    total: number;
...
export interface AddRecipientsPayload {
  leadIds?: string[];
  search?: string;
  status?: string;
  source?: string;
