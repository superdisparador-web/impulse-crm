...
export class AnalyticsController {
  ...
  @Get('executive') @Permissions('analytics.dashboard.read') executive(@Query() query: AnalyticsQueryDto, @Req() req: AuthRequest) { return this.analyticsService.executive(query, req.user!); }
  @Get('campaigns/:id/funnel') @Permissions('analytics.campaign.read') funnel(@Param('id') id:string,@Query() query:AnalyticsQueryDto,@Req() req:AuthRequest){return this.analyticsService.funnel(id,query,req.user!);}
  @Get('templates') @Permissions('analytics.whatsapp.read') templates(@Query() query:AnalyticsQueryDto,@Req() req:AuthRequest){return this.analyticsService.templates(query,req.user!);}
  @Get('events') @Permissions('analytics.dashboard.read') events(@Query() query:EntityMetricsQueryDto,@Req() req:AuthRequest){return this.analyticsService.events(query,req.user!);}
