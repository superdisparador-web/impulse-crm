...
export class WhatsappCredentialCryptoService {
  private key(): Buffer { const secret = process.env.META_TOKEN_ENCRYPTION_KEY || process.env.SECRETS_ENCRYPTION_KEY || process.env.WHATSAPP_CREDENTIAL_SECRET; if (!secret || secret.length < 32) throw new InternalServerErrorException('META_TOKEN_ENCRYPTION_KEY obrigatório para credenciais'); return createHash('sha256').update(secret).digest(); }
