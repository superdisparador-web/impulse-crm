...
type Props = { lead?: Lead | null; users: User[]; onSuccess: () => void; onCancel: () => void };

export default function LeadForm({ lead, users, onSuccess, onCancel }: Props) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [form, setForm] = useState<LeadFormData>({ name: lead?.name ?? "", phone: lead?.phone ?? "", email: lead?.email ?? "", document: lead?.document ?? "", source: lead?.source ?? "MANUAL", status: lead?.status ?? "NEW", temperature: lead?.temperature ?? "COLD", assignedUserId: lead?.assignedUserId ?? "", notes: lead?.notes ?? "", pipelineId: lead?.pipelineId ?? null, stageId: lead?.stageId ?? null });

  function update(field: keyof LeadFormData, value: string) { setForm((current) => ({ ...current, [field]: value })); }

  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!form.name.trim() || !form.phone.trim()) { setError("Informe nome e telefone."); return; }
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><form onSubmit={submit} className="max-h-[90vh] w-full max-w-3xl space-y-4 overflow-y-auto rounded-xl bg-slate-900 p-6">
    <div><h2 className="text-2xl font-bold">{lead ? "Editar lead" : "Novo lead"}</h2><p className="text-sm text-slate-400">Nome e telefone são obrigatórios. A organização é definida pelo usuário logado.</p></div>
    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <select className="rounded-lg bg-slate-800 p-3" value={form.assignedUserId ?? ""} onChange={(e) => update("assignedUserId", e.target.value)}><option value="">Sem responsável</option>{users.map((user) => <option key={user.id} value={user.id}>{user.name}</option>)}</select>
    </div>
