...
export default function LegacyLeadHistory({
  history = [],
}: {
  history?: LeadEvent[];
}) {
      ...
      <ol className="space-y-2">
        {stages.map((stage, index) => (
          <li key={stage.id} className="flex items-center gap-3">
            <div
              className={`h-3 w-3 rounded-full ${stage.current ? "bg-blue-400" : "bg-slate-600"}`}
            />
            <div>
              <p
                className={
                  stage.current ? "font-semibold text-white" : "text-slate-300"
                }
              >
                {stage.name}
              </p>
              ...
              {index < stages.length - 1 && <p className="text-slate-600">↓</p>}
            </div>
          </li>
        ))}
      </ol>
