...
export function LeadActivities({
  ...
  saving: boolean;
  onCreate: (data: LeadActivityFormData) => void;
}) {
  const [title, setTitle] = useState(options[0]);
  const [dueAt, setDueAt] = useState("");
  const [note, setNote] = useState("");
  function submit() {
    if (!dueAt || !responsibleUserId) return;
    onCreate({
      title,
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="grid gap-2 rounded-xl border border-slate-800 p-3">
        ...
        <Button
          onClick={submit}
          disabled={saving || !dueAt || !responsibleUserId}
        ...
        {!responsibleUserId && (
          <p className="text-xs text-amber-300">
            Defina um corretor responsável para criar atividades.
          </p>
        )}
      </div>
