...
export function LeadDrawer({ card, board, onClose, onArchived }: { card: PipelineCard | null; board: PipelineBoard | null; onClose: () => void; onArchived: () => void }) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      void getLead360(card.lead.id).then((data) => {
        if (!active) return;
  ...
  const timeline = useMemo(() => lead && card ? mapLeadTimeline({ lead, card, events: lead.events ?? [], activities, notes }) : [], [lead, card, activities, notes]);
  ...
  async function persistNotes(nextNotes: LeadNote[]) {
    ...
    try { const updated = await updateLead360(lead.id, { notes: serializeLeadNotes(nextNotes), organizationId: lead.organizationId }); setLead(updated); setNotes(nextNotes); }
