...
export default function UserForm({ user, organizations, saving, canAssignOrganizations, canAssignRoles, onCancel, onSubmit }: UserFormProps) {
  ...
  return <form className="space-y-5" onSubmit={handleSubmit}>
    {error && <p className="ds-radius-control border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}
    ...
    <div className="flex justify-end gap-3 border-t ds-border pt-6"><Button type="button" variant="secondary" onClick={onCancel}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? "Salvando..." : "Salvar"}</Button></div>
  </form>;
}
