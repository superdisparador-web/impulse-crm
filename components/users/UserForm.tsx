...
interface UserFormProps { user?: User | null; saving: boolean; onCancel: () => void; onSubmit: (data: UserFormData) => Promise<void>; }

export default function UserForm({ user, saving, onCancel, onSubmit }: UserFormProps) {
  const [error, setError] = useState("");
  const [formData, setFormData] = useState<UserFormData>({ name: user?.name ?? "", email: user?.email ?? "", password: "", phone: user?.phone ?? "", role: user?.role ?? "CORRETOR", active: user?.active ?? true });
  ...
  return <form className="space-y-5" onSubmit={handleSubmit}>
    ...
    <div className="grid gap-5 md:grid-cols-2"><Select label="Papel" value={formData.role} onChange={(event) => setFormData({ ...formData, role: event.target.value as UserRole })} options={[{ label: "Administrador", value: "ADMIN" }, { label: "Corretor", value: "CORRETOR" }]} /></div>
