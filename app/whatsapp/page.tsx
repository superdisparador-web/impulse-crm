...
export default function WhatsappPage() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => { const params = new URLSearchParams(window.location.search); const result = params.get("signup"); if (!result) return; if (result === "success") { setNotice("Conta conectada com sucesso. Números e templates foram sincronizados."); void load(true); } else setError(friendlySignupErrors[result] ?? friendlySignupErrors.failed); window.history.replaceState({}, "", "/whatsapp"); }, 0); return () => window.clearTimeout(timer); }, [load]);
