...
import { createPipelineStage, deletePipelineStage, getPipelineBoard, listPipelines, movePipelineCard, reorderPipelineStages, updatePipelineStage } from "@/services/pipeline-board.service";
import { PipelineBoard, PipelineFilters as Filters, PipelineStage, PipelineSummary } from "@/types/pipeline-board";
...
export default function PipelinePage() {
  ...
  useEffect(() => { const timeoutId = window.setTimeout(() => setAppliedFilters((current) => current.search === filters.search ? current : { ...current, search: filters.search }), 300); return () => window.clearTimeout(timeoutId); }, [filters.search]);
  ...
  async function mutateStages(action: () => Promise<unknown>) { if (!selectedPipelineId || savingStage) return; setSavingStage(true); setMoveError(""); try { await action(); await loadBoard(selectedPipelineId, true); } catch (err) { setMoveError(getErrorMessage(err)); } finally { setSavingStage(false); } }
  const createStage = (name: string, color: string) => mutateStages(() => createPipelineStage(selectedPipelineId, { name, color }));
  const updateStage = (stage: PipelineStage, name: string, color: string) => mutateStages(() => updatePipelineStage(selectedPipelineId, stage.id, { name, color }));
  const deleteStage = (stage: PipelineStage) => mutateStages(() => deletePipelineStage(selectedPipelineId, stage.id));
  const reorderStages = (stages: PipelineStage[]) => mutateStages(async () => { setBoard((current) => current ? { ...current, stages: stages.map((stage, index) => ({ ...stage, position: index + 1 })) } : current); await reorderPipelineStages(selectedPipelineId, stages); });

  const filterSuggestions = useMemo(() => ({
    broker: [...new Set(board?.stages.flatMap((stage) => stage.cards.map((card) => card.lead.assignedUser?.name).filter((name): name is string => Boolean(name))) ?? [])],
    manager: [...new Set(board?.stages.flatMap((stage) => stage.cards.map((card) => card.lead.managerUser?.name).filter((name): name is string => Boolean(name))) ?? [])],
    campaign: [...new Set(board?.stages.flatMap((stage) => stage.cards.map((card) => card.lead.campaign).filter((name): name is string => Boolean(name))) ?? [])],
    product: [...new Set(board?.stages.flatMap((stage) => stage.cards.map((card) => card.lead.product ?? card.lead.development).filter((name): name is string => Boolean(name))) ?? [])],
  }), [board]);

  const isLoading = loadingPipelines || loadingBoard;
  const selectedCard = useMemo(() => board?.stages.flatMap((stage) => stage.cards.map((card) => ({ ...card, stageId: stage.id }))).find((card) => card.id === selectedCardId) ?? null, [board, selectedCardId]);

  return <main className="space-y-6"><PipelineHeader pipelines={pipelines} selectedPipelineId={selectedPipelineId} loading={isLoading} refreshing={refreshing} onSelectPipeline={(pipelineId) => { void selectPipeline(pipelineId); }} onRefresh={() => { if (selectedPipelineId) void loadBoard(selectedPipelineId, true); }} /><PipelineFilters value={filters} suggestions={filterSuggestions} onChange={setFilters} onApply={() => setAppliedFilters(filters)} onClear={() => { const clean: Filters = { sla: "ALL", limit: 50 }; setFilters(clean); setAppliedFilters(clean); }} /><PipelineMetrics metrics={board?.metrics} stages={board?.stages} /><StageManager stages={board?.stages ?? []} saving={savingStage} error={moveError} onCreate={createStage} onUpdate={updateStage} onDelete={deleteStage} onReorder={reorderStages} /><PipelineBody error={error} moveError={moveError} isLoading={isLoading} pipelineCount={pipelines.length} board={board} activeCardId={activeCardId} moving={Boolean(movingCardId)} onDragStart={setActiveCardId} onDropCard={(target) => { void handleMove(target); }} onOpenCard={setSelectedCardId} /><LeadDrawer card={selectedCard} board={board} onClose={() => setSelectedCardId("")} onArchived={() => { if (selectedPipelineId) void loadBoard(selectedPipelineId, true); }} /></main>;
}
