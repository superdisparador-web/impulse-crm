...
export default function OrganizationsPage() {
  ...
  const canManageOrganizations =
    typeof window !== "undefined" && isGlobalAdmin();
  ...
  const pageMetrics = useMemo(
    () => ({
      active: organizations.filter((organization) => organization.active)
        .length,
      inactive: organizations.filter((organization) => !organization.active)
        .length,
      users: organizations.reduce(
        (sum, organization) => sum + (organization._count?.users ?? 0),
        0,
      ),
    }),
    [organizations],
  ...
  async function handleToggleStatus(organization: Organization) {
    ...
    try {
      setSaving(true);
      const updated = await organizationService.updateStatus(
        organization.id,
        !organization.active,
      );
      setSuccess(
      ...
      console.error(err);
      setError(
        getErrorMessage(err, "Não foi possível alterar o status da empresa."),
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(organization: Organization) {
    if (
      ...
      saving ||
      !confirm(`Deseja excluir a empresa ${organization.name}?`)
    )
      return;
    try {
  ...
  return (
    <main className="space-y-6 text-slate-900">
      ...
      >
        ...
        />
        <StatCard
          title="Ativas nesta página"
          value={pageMetrics.active}
          icon={<BuildingIcon size={22} />}
        />
        <StatCard
          title="Inativas nesta página"
          value={pageMetrics.inactive}
          icon={<BuildingIcon size={22} />}
        />
        <StatCard
          title="Usuários nesta página"
          value={pageMetrics.users}
      ...
      <Card padding="md">
        <div className="grid items-end gap-4 md:grid-cols-[minmax(280px,1fr)_220px]">
          <div className="relative">
            <Search
              className="pointer-events-none absolute bottom-3.5 left-4 z-10 text-slate-400"
              size={18}
            />
            <Input
      ...
      {canManageOrganizations && (
        <Modal
          isOpen={isFormOpen}
          title={selectedOrganization ? "Editar empresa" : "Nova empresa"}
