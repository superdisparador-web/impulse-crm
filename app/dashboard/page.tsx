          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <header className="max-w-md lg:max-w-[42%]">
              ...
              <p className="mt-3 text-sm font-semibold tracking-wide text-cyan-100 sm:text-base">
                {currentDateTime ? getGreeting(currentDateTime, userName) : "Olá, Rodrigo 👋"}
              </p>
              ...
              <p className="mt-2 max-w-sm text-sm leading-6 text-slate-200 sm:text-base">
                Seu centro de comando para campanhas, WhatsApp e gestão de leads.
              </p>
            </header>
            ...
            >
              ...
              {currentDateTime ? (
                <div className="mt-2 grid grid-cols-[1fr_auto] items-end gap-4">
                  <div>
                    <p className="font-semibold capitalize text-white">{formatCurrentWeekday(currentDateTime)}</p>
                    <p className="mt-0.5 text-sm capitalize text-slate-300">{formatCurrentDate(currentDateTime)}</p>
                  </div>
                  <p className="text-2xl font-bold tabular-nums text-white transition-transform duration-300 group-hover:scale-[1.03] motion-reduce:transition-none">{formatCurrentTime(currentDateTime)}</p>
                </div>
              ) : (
                <p className="mt-3 text-sm text-slate-300">Atualizando data e hora…</p>
              )}
            </aside>
          </div>

          <div className="grid grid-cols-2 overflow-hidden rounded-t-2xl border-x border-t border-white/15 bg-blue-950/65 shadow-[0_-10px_30px_-20px_rgba(34,211,238,0.35)] backdrop-blur-xl sm:grid-cols-4" aria-label="Resumo do dashboard">
            <article className="border-b border-white/10 p-3 transition-colors duration-200 hover:bg-cyan-300/[0.06] motion-reduce:transition-none sm:border-b-0 sm:p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-300"><Users className="h-4 w-4 text-cyan-300" aria-hidden="true" />Leads na semana</div>
              <p className="mt-2 text-2xl font-bold text-white">{formatNumber(dashboard.summary.leadsThisWeek)}</p>
              <p className="mt-1 text-xs text-slate-300">Novos contatos recebidos</p>
            </article>
            ...
            <article className="border-b border-l border-white/10 p-3 transition-colors duration-200 hover:bg-cyan-300/[0.06] motion-reduce:transition-none sm:border-b-0 sm:p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-300"><Megaphone className="h-4 w-4 text-cyan-300" aria-hidden="true" />Campanhas</div>
              <p className="mt-2 text-2xl font-bold text-white">{formatNumber(processingCampaigns)} <span className="block text-xs font-medium text-slate-300 xl:inline xl:text-sm">em processamento</span></p>
              <p className="mt-1 text-xs text-slate-300">{formatNumber(dashboard.summary.scheduledCampaigns)} agendadas • {formatNumber(pausedCampaigns)} pausadas</p>
            </article>
            ...
            <article className="p-3 transition-colors duration-200 hover:bg-cyan-300/[0.06] motion-reduce:transition-none sm:border-l sm:border-white/10 sm:p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-300"><MousePointerClick className="h-4 w-4 text-cyan-300" aria-hidden="true" />Engajamento</div>
              <p className="mt-2 text-2xl font-bold text-white">{formatPercentage(readRate)}% <span className="block text-xs font-medium text-slate-300 xl:inline xl:text-sm">de leitura</span></p>
              <p className="mt-1 text-xs text-slate-300">{formatPercentage(deliveryRate)}% entrega • {formatPercentage(clickRate)}% cliques</p>
            </article>
            ...
            <article className="border-l border-white/10 p-3 transition-colors duration-200 hover:bg-cyan-300/[0.06] motion-reduce:transition-none sm:p-4">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-300"><Trophy className="h-4 w-4 text-cyan-300" aria-hidden="true" />Top corretor</div>
              <p className="mt-2 truncate text-xl font-bold text-white">{topUser?.name ?? "Ranking em formação"}</p>
              <p className="mt-1 text-xs text-slate-300">{topUser ? `${formatNumber(topUser.totalLeads)} lead(s) atribuído(s)` : "Aguardando leads atribuídos"}</p>
            </article>
          </div>
        </div>
      </section>
