...
export default function LoginPage() {
  ...
  return (
    <main className="login-command-center relative isolate min-h-dvh overflow-hidden bg-[#020611] text-white">
      ...
      <div
        aria-hidden="true"
        className="login-stars login-stars-far absolute inset-0"
      />
      <div
        aria-hidden="true"
        className="login-stars login-stars-near absolute inset-0"
      />
      <div
        aria-hidden="true"
        className="login-horizon-glow absolute inset-x-0 bottom-0 h-[48%]"
      />
      <div aria-hidden="true" className="login-vignette absolute inset-0" />

      <div
        aria-hidden="true"
        className="login-orbit absolute -left-48 top-[48%] hidden size-[720px] -translate-y-1/2 rounded-full xl:block"
      >
      ...
      <div
        aria-hidden="true"
        className="login-flight-path absolute left-[8%] top-[22%] hidden h-px w-[34%] -rotate-[11deg] lg:block"
      >
      ...
      <div
        aria-hidden="true"
        className="login-hud-layer absolute inset-0 hidden xl:block"
      >
      ...
      <div className="relative z-10 mx-auto grid min-h-dvh w-full max-w-[1500px] items-center gap-12 px-5 py-5 sm:px-8 lg:grid-cols-[minmax(0,1fr)_460px] lg:px-14 lg:pb-8 lg:pt-32 xl:gap-24 xl:px-20 xl:pt-[172px]">
        ...
        >
          ...
          <h2 className="max-w-[620px] text-[46px] font-medium leading-[1.07] tracking-[-0.045em] text-white xl:text-[60px]">
            Tecnologia que{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-200 via-blue-300 to-indigo-300">
              impulsiona
            </span>{" "}
            resultados.
          </h2>
          ...
          <div className="mt-7 flex flex-wrap gap-2.5">
            {benefits.map(({ label, icon: Icon }) => (
              <div
                key={label}
                className="login-benefit-chip flex items-center gap-2 rounded-full border border-white/[0.08] bg-slate-950/35 px-3 py-2 text-[11px] text-slate-300 backdrop-blur-md"
              >
                <Icon
                  aria-hidden="true"
        ...
        <section className="brand-enter login-panel-wrap mx-auto flex w-full max-w-[440px] flex-col justify-center py-2 sm:py-6">
          ...
          >
            <div
              aria-hidden="true"
              className="login-panel-scan absolute inset-x-8 top-0 h-px bg-gradient-to-r from-transparent via-cyan-100/90 to-transparent"
            />

            <p className="text-center text-[9px] font-semibold uppercase tracking-[0.38em] text-cyan-200/65">
              Impulse Command Center
            </p>
            ...
            <div className="mt-4 text-center">
              <h1 className="text-[27px] font-semibold tracking-[-0.035em] text-white sm:text-[30px]">
                Bem-vindo de volta!
              </h1>
            ...
            <div className="mt-6">
              ...
              <div className="group relative">
                <Mail
                  aria-hidden="true"
                  className="pointer-events-none absolute left-4 top-1/2 size-[18px] -translate-y-1/2 text-slate-500 transition-colors group-focus-within:text-cyan-300"
                />
                <input
                  id="email"
                  type="email"
            ...
            <div className="mt-4">
              ...
              <div className="group relative">
                ...
                <input
                  ...
                  className="login-input h-[52px] w-full rounded-xl border border-white/[0.11] bg-slate-950/50 pl-11 pr-12 text-[15px] text-white shadow-[inset_0_1px_2px_rgba(0,0,0,0.35),inset_0_0_0_1px_rgba(255,255,255,0.015)] outline-none transition duration-200 placeholder:text-slate-500 hover:border-white/20 focus:border-cyan-300/60 focus:bg-slate-950/70 focus:ring-4 focus:ring-cyan-400/10"
                />
                <button
                  type="button"
                  ...
                  className="absolute right-2 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-lg text-slate-500 transition hover:bg-white/5 hover:text-slate-200 focus-visible:text-cyan-300"
                >
                  {showPassword ? (
            ...
            {error && (
              <p
                role="alert"
                className="mt-4 rounded-xl border border-red-300/20 bg-red-500/10 px-4 py-3 text-sm text-red-200"
              >
                {error}
              </p>
            )}
            ...
            <button
              ...
              disabled={loading}
              aria-busy={loading}
              className="login-submit group relative mt-6 flex h-[54px] w-full items-center justify-center gap-2 overflow-hidden rounded-xl border border-cyan-200/20 px-4 text-[15px] font-semibold text-white transition duration-200 hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-blue-400/25 active:translate-y-0 disabled:translate-y-0 disabled:cursor-wait disabled:opacity-60"
            >
          ...
          <div className="mt-3 grid grid-cols-3 gap-2 px-1 lg:hidden">
            {benefits.map(({ label, icon: Icon }) => (
              <div
                key={label}
                className="flex min-h-16 flex-col items-center justify-center gap-1.5 rounded-xl border border-white/[0.07] bg-slate-950/35 px-1.5 text-center backdrop-blur-sm"
              >
                <Icon aria-hidden="true" className="size-4 text-cyan-300/80" />
                <span className="text-[10px] leading-3.5 text-slate-400 sm:text-[11px]">
                  {label}
                </span>
              </div>
            ))}
