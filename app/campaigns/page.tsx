...
export default function CampaignsPage() {
  ...
  return <main className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-4xl font-bold">Campanhas</h1><p className="mt-2 text-slate-400">Crie e organize campanhas. Integração com WhatsApp, filas e disparos ficará para uma próxima etapa.</p></div><Link className="ds-radius-control ds-primary px-5 py-3 font-semibold ds-primary-hover" href="/campaigns/new">Nova Campanha</Link></div>
    ...
    <div className="grid gap-3 md:grid-cols-4"><input className="ds-radius-surface border ds-border ds-surface p-3" placeholder="Buscar por nome ou descrição" value={filters.search ?? ""} onChange={(e)=>setFilters(c=>({...c,page:1,search:e.target.value}))}/><select className="ds-radius-surface border ds-border ds-surface p-3" value={filters.status ?? ""} onChange={(e)=>setFilters(c=>({...c,page:1,status:e.target.value as CampaignStatus|""}))}><option value="">Todos os status</option>{statuses.map(s=><option key={s} value={s}>{labels[s]}</option>)}</select><select className="ds-radius-surface border ds-border ds-surface p-3" value={filters.campaignType ?? ""} onChange={(e)=>setFilters(c=>({...c,page:1,campaignType:e.target.value as CampaignType|""}))}><option value="">Todos os tipos</option>{types.map(t=><option key={t} value={t}>{labels[t]}</option>)}</select><label className="flex items-center gap-2 ds-radius-surface border ds-border ds-surface p-3"><input type="checkbox" checked={Boolean(filters.archived)} onChange={(e)=>setFilters(c=>({...c,page:1,archived:e.target.checked}))}/> Arquivadas</label></div>
    <div className="overflow-x-auto ds-radius-surface border ds-border"><table className="w-full text-sm"><thead className="ds-surface text-slate-300"><tr><th className="p-3 text-left">Nome</th><th>Status</th><th>Tipo</th><th>Filtros</th><th>Criada em</th><th>Agendamento</th><th>Ações</th></tr></thead><tbody>{loading ? <tr><td className="p-6" colSpan={7}>Carregando...</td></tr> : items.length === 0 ? <tr><td className="p-6 text-slate-400" colSpan={7}>Nenhuma campanha encontrada.</td></tr> : items.map(campaign=><tr key={campaign.id} className="border-t ds-border"><td className="p-3 font-medium">{campaign.name}</td><td><span className="rounded-full ds-surface-raised px-2 py-1">{labels[campaign.status] ?? campaign.status}</span></td><td>{labels[campaign.campaignType]}</td><td>{campaign._count?.filters ?? campaign.filters?.length ?? 0}</td><td>{new Date(campaign.createdAt).toLocaleString()}</td><td>{campaign.scheduledAt ? new Date(campaign.scheduledAt).toLocaleString() : "-"}</td><td className="flex flex-wrap gap-2 p-3"><Link className="text-blue-300" href={`/campaigns/${campaign.id}`}>Visualizar</Link><button disabled={Boolean(actionLoading)} onClick={()=>void runAction(campaign.id, campaign.archivedAt ? "Campanha restaurada do arquivo." : "Campanha arquivada.", ()=>campaignsService.archiveCampaign(campaign.id, !campaign.archivedAt))}>{campaign.archivedAt ? "Desarquivar" : "Arquivar"}</button><button disabled={Boolean(actionLoading)} onClick={()=>void runAction(campaign.id,"Campanha cancelada.",()=>campaignsService.cancelCampaign(campaign.id))}>Cancelar</button><button className="text-red-300" disabled={Boolean(actionLoading)} onClick={()=>{ if(!window.confirm(`Excluir ${campaign.name}?`)) return; void runAction(campaign.id,"Campanha excluída.",()=>campaignsService.deleteCampaign(campaign.id));}}>Excluir</button></td></tr>)}</tbody></table></div>
    <div className="flex justify-between text-sm text-slate-400"><span>{meta.total} campanha(s) • página {meta.page} de {meta.totalPages || 1}</span><div className="flex gap-2"><button disabled={meta.page <= 1} className="rounded ds-surface-raised px-3 py-2 disabled:opacity-50" onClick={()=>setFilters(c=>({...c,page:(c.page??1)-1}))}>Anterior</button><button disabled={meta.page >= meta.totalPages} className="rounded ds-surface-raised px-3 py-2 disabled:opacity-50" onClick={()=>setFilters(c=>({...c,page:(c.page??1)+1}))}>Próxima</button></div></div>
  </main>;
}
