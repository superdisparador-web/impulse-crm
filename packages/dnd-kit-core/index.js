...
export function useDraggable({ id, data }) {
  ...
  return {
    ...
    setNodeRef: () => {},
    transform:
      ctx?.active?.id === id && ctx.point
        ? { x: ctx.point.x, y: ctx.point.y }
        : null,
    isDragging: ctx?.active?.id === id,
  };
}
...
export function DragOverlay({ children }) {
  const ctx = useContext(Context);
  if (!ctx?.active || !ctx.point || typeof document === "undefined")
    return null;
  return createPortal(
    React.createElement(
      "div",
...
export function useSensor(sensor, options) {
